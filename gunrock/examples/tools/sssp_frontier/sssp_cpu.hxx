...
namespace sssp_cpu {
	...
	float bellman_ford(edge_t* row_offsets, vertex_t* column_indices, weight_t* nonzero_values,
		vertex_t& number_of_vertices, vertex_t& single_source, weight_t* distances, float* frontiers_size) {
		...
		for (int i = 0; i < (number_of_vertices - 1) && !exit; i++){
			...
			for (vertex_t v = 0; v < number_of_vertices; v++){
				if(frontier[v]){
					int degree = row_offsets[v+1] - row_offsets[v];
					frontiers_size[i] += degree;
				}
				exit &= !new_frontier[v];
		...
		double frontier_sum = 0;
		
		for (vertex_t i = 0; i < number_of_vertices; i++){
			frontier_sum += frontiers_size[i];
		}
