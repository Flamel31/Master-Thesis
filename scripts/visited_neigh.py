...
gunrock_command = "./gunrock/build/bin/sssp_visited %s %d >> %s"
    ...
    if len(sys.argv) >= 3:
        ...
        for line in f.readlines():
            ...
            if not os.path.exists(csv_path):
                os.mkdir(csv_path)
            dataset_csv = os.path.join(csv_path,dataset_name+"_visited.csv")
            if not os.path.exists(dataset_csv): 
                # Taking time for the dataset
                print("Taking visited neigh for %s" % dataset_name)
                os.system(gunrock_command % (dataset_full_path,rep,dataset_csv))
        f.close()
    else:
        print("Correct usage: python visited_neigh.py dataset_list num_repetition")
